...
export const generatePdf = (data: QuotationData) => {
    ...
    autoTable(doc, {
        startY: yPos,
        head: [['Product', 'Model', 'Features', 'Qty', 'Rate', 'GST Amount', 'Amount']],
        body: tableData,
        theme: 'grid',
        headStyles: {
    ...
    // Totals
    const subTotal = data.products.reduce((acc, p) => acc + p.quantity * p.rate, 0);
    const gstBreakdown: { [rate: string]: number } = data.products.reduce((acc, product) => {
      const amount = product.quantity * product.rate;
      const gstAmount = (amount * product.gstRate) / 100;
      const rateKey = product.gstRate.toString();
  
      if (!acc[rateKey]) {
        acc[rateKey] = 0;
      }
      acc[rateKey] += gstAmount;
      return acc;
    }, {} as { [rate: string]: number });
  
    const totalGst = Object.values(gstBreakdown).reduce((sum, amount) => sum + amount, 0);
    const grandTotal = subTotal + totalGst + data.freight;
    ...
    Object.entries(gstBreakdown).forEach(([rate, amount]) => {
      doc.text(`GST @ ${rate}%:`, totalsLabelX, yPos, { align: 'right' });
